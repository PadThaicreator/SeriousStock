...
export const SellCard  = ( prop : any) =>{
    const { order } = prop;
    return(
        <div className="flex flex-col  bg-white rounded-xl shadow-lg border border-gray-200 p-6 hover:shadow-xl transition-all duration-300 hover:scale-[1.02]">
            ...
            <div className="flex  flex-col space-y-4">
               ...
               {/* Port */}
                ...
                {/* Quote */}
                <div className="flex flex-col bg-blue-50 rounded-lg p-4">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-lg font-semibold blue-red-900">{order.amountSell}</p>
                        </div>
                        <div>
                            <p className="text-xs text-blue-600 uppercase tracking-wide">Quote Price</p>
                            <p className="text-lg font-semibold text-blue-900">฿{(order?.priceSell/order?.amountSell).toLocaleString()}</p>
                        </div>
                    </div>
                </div>
                
                {/* Price */}
                <div className=" flex flex-col bg-red-50 rounded-lg p-4">
                    <div className="flex items-center gap-2 mb-2">
                        <DollarSign className="w-4 h-4 text-red-600" />
                        <span className="text-sm font-medium text-red-800">Total Sell Price</span>
                    </div>
                    <p className="text-2xl font-bold text-red-900">฿{order.priceSell?.toLocaleString()}</p>
                </div>
                
                {/* CreateAt */}
                <div className="flex items-center gap-2 pt-2 border-t border-gray-100">
                    <Calendar className="w-4 h-4 text-gray-500" />
                    <span className="text-sm text-gray-600">Sell Order At:</span>
